    ...
    <div class="float-right">
        <ul class="pagination">
            <li class="mr-2 page-item {{ ($paginator->currentPage() == 1) ? ' disabled' : '' }}">
                <a class="page-link" href="#" wire:click="setPage('{{ $paginator->url(1) }}')">First</a>
            </li>
            <li class="page-item {{ ($paginator->currentPage() == 1) ? ' disabled' : '' }}">
                <a class="page-link" href="#" wire:click="setPage('{{ $paginator->url($paginator->currentPage() - 1) }}')"><em class="fa fa-caret-left"></em></a>
            </li>
            {{-- Put a dot --}}
            ...
            @for ($i = 1; $i <= $paginator->lastPage(); $i++)
                ...
                @if ($from < $i && $i < $to)
                    <li class="page-item {{ ($paginator->currentPage() == $i) ? ' active' : '' }}">
                        <a class="page-link" href="#" wire:click="setPage('{{ $paginator->url($i) }}')">{{ $i }}</a>
                    </li>
                @endif
            @endfor
            {{-- Put a dots --}}
            ...
            <li class="page-item {{ ($paginator->currentPage() == $paginator->lastPage()) ? ' disabled' : '' }}" disabled="true">
                <a class="page-link" href="#" wire:click="setPage('{{ $paginator->url($paginator->currentPage() + 1) }}')"><em class="fa fa-caret-right"></em></a>
            </li>
            <li class="ml-2 page-item {{ ($paginator->currentPage() == $paginator->lastPage()) ? ' disabled' : '' }}">
                <a class="page-link" href="#" wire:click="setPage('{{ $paginator->url($paginator->lastPage()) }}')">Last</a>
            </li>
        </ul>
    </div>
@endif